#!/usr/bin/env python3
"""
Script RÁPIDO para buscar imágenes - solo primeros 10 productos
Útil para testing y validación rápida
"""
...
def get_products_from_supabase(limit: int = 10) -> List[Dict]:
    """Obtiene primeros N productos de Supabase."""
    try:
        from supabase import create_client
        supabase = create_client(SUPABASE_URL, SUPABASE_KEY)
        result = supabase.table("productos").select("id_producto, nombre").execute()
        products = (result.data or [])[:limit]
        logger.info(f"✓ Se obtuvieron {len(products)} productos (limite: {limit})")
        return products
...
def _generate_placeholder_urls(product_name: str, num_images: int = 3) -> List[str]:
    ...
    # Servicios que siempre funcionan:
    # - picsum.photos - Random photos from Lorem Picsum
    # - via.placeholder.com - Color blocks
    # - loremflickr.com - Random Flickr photos
...
def create_excel_with_images(products: List[Dict], output_file: str = "product_images_test.xlsx"):
    """Crea Excel con URLs de imágenes."""
    ...
    for idx, product in enumerate(products, start=1):
        ...
        print(f"[{idx:2d}/{len(products)}] {nombre[:40]:<40} ", end="", flush=True)
        ...
        # Validar URLs
        if image_urls:
            print(f"({len(image_urls)} URLs) validando...")
            valid_urls = validate_image_urls(image_urls)
            print(f"✓ {len(valid_urls)} válidas")
        else:
            valid_urls = []
            print(f"sin URLs encontradas")
...
def main():
    logger.info("="*80)
    logger.info("AUTO SEARCH IMAGES (TEST) - Versión rápida con 10 productos")
    logger.info("="*80)
    logger.info("")
    
    products = get_products_from_supabase(limit=10)
    if not products:
        logger.error("No se encontraron productos")
        return
    
    create_excel_with_images(products, output_file="product_images_test.xlsx")

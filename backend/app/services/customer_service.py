...
class CustomerService:
    ...
    async def get_customer_addresses(self, customer_id: UUID) -> List[Any]:
        """Obtiene todas las direcciones de un cliente."""
        from app.models.schemas import AddressResponse

        rows = await self._db.get_customer_addresses(customer_id)
        addresses = []
        for row in rows:
            addresses.append(AddressResponse(
                id=UUID(str(row["id_direccion"])),
    ...
    async def create_address(self, customer_id: UUID, request: Any) -> Any:
        ...
        if request.isPrimary:
            existing = await self._db.get_customer_addresses(customer_id)
            for addr in existing:
                if addr.get("es_principal"):
                    await self._db.update_address(
                        UUID(str(addr["id_direccion"])),
                        {"es_principal": False}
                    )

        address_id = await self._db.create_address(db_data)
    ...
    async def update_address(self, customer_id: UUID, address_id: UUID, request: Any) -> Any:
        """Actualiza una dirección existente."""
        from app.models.schemas import AddressResponse

        existing = await self._db.get_address_by_id(address_id)
        if not existing:
            raise DatabaseError("Dirección no encontrada")

        db_data = {}
        if request.label is not None:
            db_data["etiqueta"] = request.label
        if request.street is not None:
            db_data["calle"] = request.street
        if request.city is not None:
            db_data["ciudad"] = request.city
        if request.province is not None:
            db_data["provincia"] = request.province
        if request.zip is not None:
            db_data["codigo_postal"] = request.zip
        if request.isPrimary is not None:
            db_data["es_principal"] = request.isPrimary
            # If setting as primary, unset other primary addresses
            if request.isPrimary:
                addresses = await self._db.get_customer_addresses(customer_id)
                for addr in addresses:
                    if addr.get("es_principal") and UUID(str(addr["id_direccion"])) != address_id:
                        await self._db.update_address(
                            UUID(str(addr["id_direccion"])),
                            {"es_principal": False}
                        )

        await self._db.update_address(address_id, db_data)

        # Return updated address
        updated = await self._db.get_address_by_id(address_id)
        if not updated:
            raise DatabaseError("Dirección actualizada pero no se pudo leer")

        return AddressResponse(
            id=UUID(str(updated["id_direccion"])),
            label=updated.get("etiqueta", ""),
            street=updated.get("calle", ""),
            city=updated.get("ciudad", ""),
            province=updated.get("provincia", ""),
            zip=updated.get("codigo_postal", ""),
            isPrimary=bool(updated.get("es_principal", False)),
        )

    async def delete_address(self, address_id: UUID) -> None:
        """Elimina una dirección."""
        await self._db.delete_address(address_id)

    # ------------------------------------------------------------------ #
    # Favoritos (Favorites)
    # ------------------------------------------------------------------ #

    async def get_customer_favorites(self, customer_id: UUID) -> List[Any]:
        """Obtiene todos los favoritos de un cliente."""
        from app.models.schemas import FavoriteResponse

        rows = await self._db.get_customer_favorites(customer_id)
        favorites = []
        for row in rows:
            created_at = row.get("fecha_creacion", "")
            if hasattr(created_at, "isoformat"):
                created_at = created_at.isoformat()
            favorites.append(FavoriteResponse(
                productId=UUID(str(row["id_producto"])),
                addedAt=str(created_at),
            ))
        return favorites

    async def add_favorite(self, customer_id: UUID, product_id: UUID) -> Any:
        """Agrega un producto a favoritos."""
        from app.models.schemas import FavoriteResponse
        from datetime import datetime, timezone

        await self._db.add_favorite(customer_id, product_id)
    ...
    async def remove_favorite(self, customer_id: UUID, product_id: UUID) -> None:
        ...
        await self._db.remove_favorite(customer_id, product_id)

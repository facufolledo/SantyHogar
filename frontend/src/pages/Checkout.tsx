...
const Checkout = () => {
  ...
  const [shippingAddress, setShippingAddress] = useState({
    street: '',
    city: '',
    province: 'Córdoba', // Solo operamos en Córdoba
    zip: '',
  });
  ...
  const handleAddressChange = (addressId: string) => {
    ...
    if (addressId === 'new') {
      // Limpiar formulario para nueva dirección (mantener Córdoba como provincia)
      setShippingAddress({
        street: '',
        city: '',
        province: 'Córdoba',
              ...
              <form onSubmit={handleFormSubmit} className="lg:col-span-2 space-y-4">
                ...
                <div className="card p-6">
                  <div className="flex items-center justify-between mb-4">
                    <h2 className="font-bold text-gray-900">Dirección de envío</h2>
                  </div>
                  {/* Aviso de cobertura */}
                  <div className="flex items-start gap-2 mb-4 bg-amber-50 border border-amber-200 rounded-xl px-4 py-3 text-sm text-amber-800">
                    <MapPin size={15} className="text-amber-600 mt-0.5 flex-shrink-0" />
                    <p>
                      <span className="font-semibold">Cobertura: solo Córdoba.</span>{' '}
                      Por el momento los envíos y retiros están disponibles únicamente para la provincia de Córdoba.
                    </p>
                  </div>

...
export const AuthProvider = ({ children }: { children: React.ReactNode }) => {
  ...
  const login = async (email: string, password: string) => {
    try {
      ...
      if (data.user) {
        ...
        const role: UserRole = data.user.user_metadata?.role === 'admin' ? 'admin' : 'cliente';
        return { ok: true, role };
      }
  ...
  const loginWithGoogle = async () => {
    try {
      // Usar window.location.origin para que funcione tanto en local como en producción
      // En producción: https://santyhogar.com.ar
